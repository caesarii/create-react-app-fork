...
/**
 * @param {*} name 初始化项目时 create-react-app <name> 的 name 值
 * @param {*} verbose 配置 commander option, 打印本地信息
 * @param {*} version commander option, 指定 react-scripts 的版本
 * @param {*} useNpm commander option, 是否使用 npm, 默认使用 yarn
 * @param {*} usePnp
...
function createApp(name, verbose, version, useNpm, usePnp, template) {
  ...
  // 检查 node 版本, 如果 node 版本太低则降低 react-scripts 版本
  ...
  // 使用 yarn, 将 yarn.lock.cached 复制到 yarn.lock
  if (useYarn) {
    fs.copySync(
